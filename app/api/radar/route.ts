...
export async function POST(request: Request) {
  try {
    ...
    // Usamos o endpoint "run-sync-get-dataset-items". 
    // Ele bloqueia a execução até o robô terminar e já cospe o ouro direto.
    const runRes = await fetch(
      `https://api.apify.com/v2/acts/pratikdani~tiktok-shop-search-scraper/run-sync-get-dataset-items?token=${API_TOKEN}`,
      {
        ...
        body: JSON.stringify({ 
          ...
          "limit": 10 // Comece com pouco para testar a velocidade
        })
      }
    );
